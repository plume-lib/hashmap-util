...
tasks.withType<JavaCompile>().configureEach {
  options.errorprone {
    ...
    // Code copied from the JDK that we do not want to change gratuitously.
...
//
// This is a method of an object rather than a top-level function of the build script, because a
// top-level function is compiled to an instance method of the script class, so a task action that
// called it would capture the script object, which the configuration cache cannot serialize.
object JavadocFonts {
...
tasks.withType<Javadoc>().configureEach {
  ...
  options.quiet()
  exclude("org/plumelib/util/WeakIdentityHashMap.java")
...
val requireJavadoc =
  tasks.register<JavaExec>("requireJavadoc") {
    ...
    args(
      sourceSets.main.get().allJava.srcDirs.map { it.absolutePath } +
        "--dont-require=WeakHasherMap|WeakIdentityHashMap"
    )
    jvmArgs(addExportsArgs)
    jvmArgs(addOpensArgs)
    // A local variable, because a task action that read `requireJavadocMarker` directly would
    // capture the build script object, which the configuration cache cannot serialize.
...
mavenPublishing {
  ...
  pom {
    name = "HashMap Util"
    description = "HashMap utility libraries for Java."
